...
struct RayCastData {
	...
	__device__
	bool trilinearInterpolationSimpleFastFast(const VoxelHashData& hash, const float3& pos, float& dist, uchar3& color) const {
	...
	// d0 near, d1 far
	__device__
		bool findIntersectionBisection(const VoxelHashData& hash, const float3& worldCamPos, const float3& worldDir, float d0, float r0, float d1, float r1, float& alpha, uchar3& color) const
	{
...
#pragma unroll 1
	...
	__device__
	float3 gradientForPoint(const VoxelHashData& hash, const float3& pos) const
	{
	...
	__device__
	void traverseCoarseGridSimpleSampleAll(const VoxelHashData& hash, const DepthCameraData& cameraData, const float3& worldCamPos, const float3& worldDir, const float3& camDir, const int3& dTid, float minInterval, float maxInterval) const
	{
