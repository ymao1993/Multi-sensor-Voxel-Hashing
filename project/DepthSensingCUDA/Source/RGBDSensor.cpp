//
// RGBDSensor provides functionalities to manage the capture process, including storing the
// camera matrices, captured result, converting depth map to point cloud, several 
// IO operations and so forth.
//
...
/**
 * Init the RGBDSensor by initializing the depth ring buffer and color buffer.
 */
...
/** initialize depth intrinsics
 * Initialize intrinsics with fx, fy, cx and cy.
 * fx=f/px, fy=f/py, cx=c/px, cy=c/py.
 */
void RGBDSensor::initializeDepthIntrinsics(float fx, float fy, float cx, float cy)
{
 ...
 * fx=f/px, fy=f/py, cx=c/px, cy=c/py.
 */
void RGBDSensor::initializeColorIntrinsics(float fx, float fy, float cx, float cy)
{
...
/**
 * savePointCloud (deprecated, use saveRecordedPointCloud() instead)
 * convert the depth image into a point cloud in the camera space, and store it on disk in .ply format.
 */
void RGBDSensor::savePointCloud( const std::string& filename, const mat4f& transform /*= mat4f::identity()*/ ) const
{
	...
	for (unsigned int i = 0; i < getDepthWidth()*getDepthHeight(); i++) {
		...
		if (d != 0.0f && d != -std::numeric_limits<float>::infinity()) {
			...
			// compute the camera space position corresponding to the depth value.
			// notice that the depth value captured is just the z value in camera space.
			// So we can the convert the screen coordinate u,v back into the camera space
			// by scaling simply scaling it by d, then pre-multiply it by the inverse 
			// intrinsic matrix.
...
/**
 * recordFrame
 * record the depth and color of the current frame
 * if m_bUseModernSensFilesForRecording is enabled, this will
 * also record the camera intrinsic and R|t matrix.
 */
void RGBDSensor::recordFrame()
{
	if (m_bUseModernSensFilesForRecording) {
		...
		// copy the color and depth buffer.
...
/**
 * Record the transformation from the current camera pose to the reference camera pose.
 */
...
/**
 * depthToSkeleton
 * convert from depth value to 3d position
 * used by getNormal()
 */
...
/**
 * compute the point cloud of the current depth map
 */
void RGBDSensor::computePointCurrentPointCloud(PointCloudf& pc, const mat4f& transform /*= mat4f::identity()*/) const
{
	...
	//
	// When performing transformation on the normal, always use the inverse transpose approach
	//
...
/**
 * Save all the point clouds recorded and save them in a single file
 */
