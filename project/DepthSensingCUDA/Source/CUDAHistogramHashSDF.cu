...
__global__ void computeHistrogramKernel(unsigned int* d_data, VoxelHashData voxelHashData)
{
	const HashParams& hashParams = c_hashParams;
	const unsigned int idx = blockIdx.x*blockDim.x + threadIdx.x;
	
	if (idx < hashParams.m_hashNumBuckets) {
		unsigned int numEntiresPerBucket = voxelHashData.getNumHashEntriesPerBucket(idx);
		atomicAdd(&d_data[numEntiresPerBucket], 1);
		unsigned int listLength = voxelHashData.getNumHashLinkedList(idx);
...
extern "C" void computeHistogramCUDA(unsigned int* d_data, const VoxelHashData& voxelHashData, const HashParams& hashParams) 
{
	...
	computeHistrogramKernel<<<gridSize, blockSize>>>(d_data, voxelHashData);
